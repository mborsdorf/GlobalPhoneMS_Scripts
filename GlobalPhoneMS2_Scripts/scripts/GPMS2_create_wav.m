...
%
%
% Modifications copyright (C) 2020 Marvin Borsdorf
...
language = {'Arabic', 'Bulgarian', 'Chinese_Mandarin', 'Chinese_Shanghai', 'Croatian', 'Czech', 'French', 'German', 'Hausa', 'Japanese', 'Korean', 'Polish', 'Portuguese', 'Russian', 'Spanish', 'Swahili', 'Swedish', 'Tamil', 'Thai', 'Turkish', 'Ukrainian', 'Vietnamese'};
 ...
 for i_lang = 1:length(language)  
    ...
    %mkdir(['D:/GlobalPhone/GP2k_Mix/Combinations/Intra_Language/GP2k_2Spk/' language{i_lang} '/wav16k']);
    %output_dir16k=['D:/GlobalPhone/GP2k_Mix/Combinations/Intra_Language/GP2k_2Spk/' language{i_lang} '/wav16k']
    ...
    for i_mm = 1:length(min_max)
        for i_type = 1:length(data_type)
            ...
            Source1File = ['PATH4/' language{i_lang} '/' language{i_lang} '_GP2k_mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_1.txt'];
            Source2File = ['PATH4/' language{i_lang} '/' language{i_lang} '_GP2k_mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_2.txt'];
            MixFile     = ['PATH4/' language{i_lang} '/' language{i_lang} '_GP2k_mix_2_spk_' min_max{i_mm} '_' data_type{i_type} '_mix.txt'];
            InfoFile    = ['PATH4/' language{i_lang} '/' language{i_lang} '_GP2k_mix_2_spk_info_' min_max{i_mm} '_' data_type{i_type} '.txt'];
